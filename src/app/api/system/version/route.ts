...
import { isTermux, applyTermuxPatches } from "@/lib/system/termuxPatch";
...
export async function GET(req: NextRequest) {
  ...
  const autoUpdateSupported = onTermux ? true : validation.supported;
  const autoUpdateError = onTermux ? null : validation.reason;

  const body = {
    current,
    latest: latest ?? "unavailable",
    updateAvailable: isNewer(latest, current),
    channel: onTermux ? "termux" : config.mode,
    autoUpdateSupported,
    autoUpdateError,
    news,
  };
...
export async function POST(req: NextRequest) {
  ...
  if (!onTermux && !validation.supported) {
    return NextResponse.json(
      {
        success: false,
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          if (onTermux) {
            try {
              applyTermuxPatches(PROJECT_ROOT);
            } catch (patchErr: any) {
              console.warn("[AutoUpdate] Termux post-build patch warning:", patchErr?.message);
            }
          }
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        if (onTermux) {
          send({ step: "install", status: "running", message: `Installing omniroute@${latest} for Termux...` });
          const termuxEnv = {
            ...process.env,
          ...
          await execFileAsync(
            "npm",
            ["install", "-g", `omniroute@${latest}`, "--global-style", "--ignore-scripts", "--no-audit", "--no-fund", "--omit=dev"],
            { cwd: PROJECT_ROOT, timeout: 300_000, env: termuxEnv }
          );
          ...
          send({
            step: "rebuild",
            status: "running",
            message: "Applying Termux compatibility patches...",
          });
          const omniPath = await resolveGlobalOmniroutePath();
          const patchStats = applyTermuxPatches(omniPath);
          send({
            step: "rebuild",
            status: "done",
            message: `Termux patches applied (playwright: ${patchStats.playwright}, bind: ${patchStats.bind}, instrumentation: ${patchStats.instrumentation})`,
          });
        } else {
